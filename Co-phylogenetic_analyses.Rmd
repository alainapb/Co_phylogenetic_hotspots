...
if(!file.exists("IvesGodfray_simplified_results.RDS")) {
  ...
  for (mod in 1:3) {
    ...
    ives_results[2*mod-1,] <- c(dataset, "brown", MSEbrown, sum(MSEbrown > brown.boot.1)/1000, sum(MSEbrown > brown.boot.2)/1000, sum(MSEbrown > brown.boot.3)/1000)
    ives_results[2*mod,] <- c(dataset, "star", MSEstar, sum(MSEstar > star.boot.1)/1000, sum(MSEstar > star.boot.2)/1000, sum(MSEstar > star.boot.3)/1000)
