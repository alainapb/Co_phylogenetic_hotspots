...
V(G)$color <- membership(MU)
G$palette <- rainbow(length(MU))
G$palette[2] <- "#6B00FF"
G$palette[29] <- "#FF2800"

set.seed(31668)
plot(MU,G,vertex.label="",vertex.size=2, layout=layout_nicely,mark.col=unname(sapply(G$palette,function(col) paste0(col,"4D"))),mark.border=unname(sapply(G$palette,function(col) paste0(col,"4D"))))
...
V(G)$color <- membership(MU)
G$palette <- rainbow(length(MU))
plot(MU,G,vertex.label="",vertex.size=2, layout=layout_nicely)
