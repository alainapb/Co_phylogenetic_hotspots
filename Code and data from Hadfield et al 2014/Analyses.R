...
data.filepath<-"./Code and data from Hadfield et al 2014/"              # file path to data files 
results.filepath<-"./Code and data from Hadfield et al 2014/"  # file path where results are to be stored 
graphs.filepath<-"./Code and data from Hadfield et al 2014/"                 # file path where graphs are to be stored 
# 
...
if(!"ndat"%in%ls()){   # read in and format data if not there
  ...
  # combinations for the analyses ignoring between-region information:
...
## What this does is create S^{-1} matrices for each term, 
## where these matrices are very sparse, with a block diagonal structure
## such that each block is the S^{-1} matrix for a particular region 
...
if(run.MCMC){
  ...
  if(incidence){
    ...
    ## NOTE: I think this is missing 'Parasite.species=parasiteA' in the ginverse argument (see mI.MCMCc, d [but not b])
    ...
    ## drop 1 term from prior b/c you cannot estimate the non-phylogenetic 
    ## interaction effect, Host.parasite.ide, without replicate incidence matrices,
    ## In this case, you can do it above because all of the between-region data is
    ## included in the contruction of the matrices. Here you can't because
